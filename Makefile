...
data/01_interim/nodes_geo.csv.bz2: data/00_original/ark.stamp awk/format_geolocation.awk | data/01_interim/
	bzip2 -cd data/00_original/$(ARK_PATH)/$(TOPOLOGY).nodes.geo.bz2 | awk -f awk/format_geolocation.awk | bzip2 -z > $@

data/01_interim/network_links.csv.bz2: data/00_original/ark.stamp awk/format_links.awk | data/01_interim/
	bzip2 -cd data/00_original/$(ARK_PATH)/$(TOPOLOGY).links.bz2 | awk -f awk/format_links.awk | bzip2 -z > $@

data/01_interim/cliques.csv.bz2: data/00_original/ark.stamp awk/format_cliques.awk | data/01_interim/
	bzip2 -cd data/00_original/$(ARK_PATH)/$(TOPOLOGY).links.bz2 | awk -f awk/format_cliques.awk | bzip2 -z > $@

data/01_interim/nodes_AS.csv.bz2: data/00_original/ark.stamp awk/format_AS.awk | data/01_interim/
	bzip2 -cd data/00_original/$(ARK_PATH)/$(TOPOLOGY).nodes.as.bz2 | awk -f awk/format_AS.awk | bzip2 -z > $@
